...
| [0016](0016-settings-finalization-no-pydantic-settings-error-split.md) | Settings finalization: no `pydantic-settings`, `ConfigurationError`/`ValidationError` split | Accepted | P8.1 continues `Settings.from_env()` plus a hand-written `.env` reader instead of adopting `pydantic-settings` (`SecretStr` and `.env` parsing are both achievable on `pydantic` alone); `configure()` wraps `pydantic.ValidationError` into `korchestrator.ValidationError` while `ConfigurationError` covers configuration-resolution failures and stays submodule-only, matching spec 04 §6's `__all__` exactly. |
| [0017](0017-agentgraph-excluded-from-json-serialization.md) | `AgentGraph` excluded from `to_json`/`from_json` | Accepted | P8.5's serializer supports `AgentState`/`ExecutionPlan`/`ModelCard`/`RunResult` only — `AgentGraph`'s nodes carry live, non-serialisable compute callables, so a byte-for-byte round trip is structurally impossible without pickling code (a security liability) or inventing an ungrounded topology-only shape no current caller needs. |

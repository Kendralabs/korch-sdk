...
| P12.3 | Supply chain | SBOM generation, checksums, build provenance attestation, license allowlist scan | P12.2 | ☐ |
| P12.4 | Publish | PyPI Trusted Publishing (OIDC, no long-lived token), gated on every prior job | P12.3 | ☐ |
| P12.5 | Release notes + docs deploy | Generated notes (supported Pythons, dependency changes, API changes, migrations, known limitations); GitHub Pages deploy | P12.4 | ☐ |
| P12.6 | Dry run | Full pipeline against a `v0.1.0-rc1` tag on TestPyPI before the real release | P12.5 | ☐ |

**Acceptance:** a tagged release builds, scans, verifies version everywhere, installs from the artifact in a clean environment, publishes immutably, and deploys docs — with no backend, frontend, container, or npm job anywhere.

**Commits:** `ci: add version validation and artifact verification [P12]` · `ci: add SBOM, provenance and trusted publishing [P12]` · `chore(release): verify pipeline via release candidate [P12]`
